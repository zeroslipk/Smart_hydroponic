...
class DatabaseService {
  ...
  Future<int> insertSensorReading(SensorReading reading) async {
    final db = await database;
    // Convert SensorReading to SensorActivity for storage
    final activity = SensorActivity(
      sensorId: reading.id,
      sensorName: reading.displayName,
      value: reading.value,
      unit: reading.unit,
  ...
  Future<SensorReading?> getLatestSensorReading(String sensorId) async {
    ...
    final activity = SensorActivity.fromMap(map);
    // Convert SensorActivity back to SensorReading
    return SensorReading(
      id: activity.sensorId,
      value: activity.value,
      unit: activity.unit,
      timestamp: activity.timestamp,
      status: activity.status,
      min: 0, // Default values - adjust based on sensor type
      max: 100,
    );
  }
  ...
  Future<List<SensorReading>> getSensorReadings(
    ...
    final activities = await getSensorActivities(
      sensorId: sensorId,
      startDate: startDate,
      endDate: endDate,
      limit: limit ?? 100,
