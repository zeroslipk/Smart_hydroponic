...
// Actuators (GPIO pins matching Wokwi diagram - D27->IN1->Fan LED, D26->IN2->Lights, D25->IN3->Pump LED)
#define PUMP_PIN 25        // Water Pump control (GPIO 25 -> IN3 -> Relay -> LED)
#define LIGHTS_PIN 26      // LED Grow Lights control (GPIO 26 -> IN2 -> Relay -> LED)
#define FAN_PIN 27         // Cooling Fan control (GPIO 27 -> IN1 -> Relay -> LED)
...
const long updateInterval = 1000; // Update every 2 seconds
const long actuatorCheckInterval = 1000; // Check actuators every 2 seconds
...
void setup() {
  ...
  Serial.println("✓ 3 Actuators initialized:");
  Serial.println("  • Pump (GPIO 25)");
  Serial.println("  • Lights (GPIO 26)");
  Serial.println("  • Fan (GPIO 27)");
...
void readActuatorCommands() {
  // Check WiFi connection
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("❌ WiFi not connected, skipping actuator read");
    return;
  }
  ...
  Serial.printf("📥 Reading actuators from: %s\n", url.c_str());
  ...
  if (httpResponseCode == 200) {
    ...
    if (!error) {
      // Control Pump (water_pump in Firebase) - GPIO 27 -> IN1
      if (doc.containsKey("water_pump") && doc["water_pump"].containsKey("state")) {
        bool pumpState = doc["water_pump"]["state"];
        digitalWrite(PUMP_PIN, pumpState ? HIGH : LOW);
        Serial.printf("🔵 Pump (GPIO %d): %s\n", PUMP_PIN, pumpState ? "ON" : "OFF");
      }
      
      // Control Lights (led_lights in Firebase) - GPIO 26 -> IN2
      if (doc.containsKey("led_lights") && doc["led_lights"].containsKey("state")) {
        bool lightsState = doc["led_lights"]["state"];
        digitalWrite(LIGHTS_PIN, lightsState ? HIGH : LOW);
        Serial.printf("💡 Lights (GPIO %d): %s\n", LIGHTS_PIN, lightsState ? "ON" : "OFF");
      }
      
      // Control Fan (cooling_fan in Firebase) - GPIO 25 -> IN3
      if (doc.containsKey("cooling_fan") && doc["cooling_fan"].containsKey("state")) {
        bool fanState = doc["cooling_fan"]["state"];
        digitalWrite(FAN_PIN, fanState ? HIGH : LOW);
        Serial.printf("🌀 Fan (GPIO %d): %s\n", FAN_PIN, fanState ? "ON" : "OFF");
      }
    } else {
      Serial.printf("⚠️  JSON parse error: %s\n", error.c_str());
      Serial.printf("⚠️  Payload was: %s\n", payload.c_str());
    }
  } else if (httpResponseCode == -1) {
    Serial.println("⚠️  Timeout reading actuators");
  } else {
    Serial.printf("❌ Failed to read actuators: HTTP %d\n", httpResponseCode);
